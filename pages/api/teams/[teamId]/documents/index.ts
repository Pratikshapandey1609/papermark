  ...
  if (req.method === "GET") {
    ...
    try {
      ...
      if (type === "docs" || type === "slides") {
        ...
        // Only trigger if TRIGGER_API_KEY is configured
        if (process.env.TRIGGER_API_KEY) {
          try {
            ...
            console.error("Failed to trigger file conversion:", error);
            // Don't fail the document creation if trigger fails
          }
        }
      }
      ...
      if (type === "cad") {
        ...
        if (process.env.TRIGGER_API_KEY) {
          try {
            ...
            console.error("Failed to trigger CAD conversion:", error);
            // Don't fail the document creation if trigger fails
          }
        }
      }
